angular.module('diveApp.visualization').controller('VisualizationsCtrl', ($scope, $rootScope, $state, $stateParams, $q, $mdToast, DataService, PropertiesService, SpecsService, pIDRetrieved) ->
  @ATTRIBUTE_TYPES =
    NUMERIC: ["int", "float"]
  ...
  @shareVisualization = ->
    ...
    $('.export-button').hide()
    return SpecsService.exportVisualization(_params).then((vID) =>
      console.log("Visualization ID:", vID)
      @visualizationUrl = "#{window.location.origin}/projects/#{$stateParams.formattedProjectTitle}/visualizations/#{vID}"
    )
  ...
  @displayToast = (text) ->
    getToastPosition = ->
      ...
      return Object.keys(_toastPosition).filter((position) ->
        _toastPosition[position]
      ).join ' '

    $mdToast.show $mdToast.simple().content(text).position(getToastPosition()).hideDelay(3000)
  ...
  @getVisualization = () ->
    ...
    SpecsService.getVisualizationDataFromSpec(_params).then((data) =>
  ...
  @errorCopying = (err) ->
    @displayToast('Error copying link to clipboard.')
    console.error err
    return

  @getVisualizationFromVID = ->
    _params = {
      vID: @vID
    }
  ...
  @getDatasets = ->
    pIDRetrieved.promise.then((r) =>
      DataService.getDatasets().then((datasets) =>
        @datasetsLoaded = true
        @datasets = datasets
        if @datasets and datasets[0]?.dID
